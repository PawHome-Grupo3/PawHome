...
public class AnimalSpecifications {
    ...
    public static Specification<Animal> nombreContiene(String keyword) {
        return (root, query, cb) -> cb.like(cb.lower(root.get("nombre")), "%" + keyword.toLowerCase() + "%");
    }

    public static Specification<Animal> adoptadoEs(Boolean adoptado) {
        return (root, query, cb) -> cb.equal(root.get("adoptado"), adoptado);
    }

    public static Specification<Animal> especieIdEs(Integer especieId) {
        return (root, query, cb) -> cb.equal(root.get("especie").get("id"), especieId);
    }
